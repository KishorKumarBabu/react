...
const menuCards = restinfo?.cards?.find(card => card?.groupedCard)?.groupedCard?.cardGroupMap?.REGULAR?.cards;
    ...
    <div className="rest-menu-container">
      ...
      <ul>
        ...
        {itemCards.map((item, index)=> (
          <li key={`${item.card.info.id || 'no-id'}-${index}` } className="menu-item" ><div className="menu-content"><h3>{item.card.info.name}</h3><h4>Rs.{(item.card.info.price)/100 || item.card.info.defaultPrice/100}</h4><h5>{item.card.info.ratings.aggregatedRating.rating}</h5><p>({item.card.info.ratings.aggregatedRating.ratingCountV2})</p><p>{item.card.info.description}</p></div>
